import React from 'react'
import styles from './index.module.css'
const Message = () => {
    return (
        <div className={styles.messageContainer}>
            <img className={styles.profileImage} src="https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/e47f43c3-d1b5-4cb6-be78-0170bca3daef/dd5lmp1-1e89ca90-4f21-4f47-9005-8d36b26ce913.jpg/v1/fill/w_800,h_800,q_75,strp/fat_toad_by_phiphiauthon_dd5lmp1-fullview.jpg?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..xbgv59kxb9WxQOXDDcA6zeVMhfXOJfvAdM6a3VCFENQ" alt="" />
            <div className={styles.messageContent}>
                <div>Big beeg</div>
                <div>Lorem ipsum dolor sit amet consectetur adipisicing elit. Incidunt optio enim ea consequatur quae tenetur, deleniti ratione reiciendis temporibus dolorem voluptas autem vero doloremque totam soluta et numquam iusto sit!</div>
            </div>
        </div>
    )
}

export default Message